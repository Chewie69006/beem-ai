...
async def async_setup_entry(
    ...
    sensors: list[SensorEntity | BinarySensorEntity] = [
        ...
        BeemAISensor(
            ...
            state_class=None,
            unit=None,
            value_fn=lambda c: "on" if c.state_store.plan.allow_grid_charge else "off",
            device_type="system",
        ),
        BeemAISensor(
            coordinator, entry,
            key="prevent_discharge",
            name="Prevent Discharge",
            icon="mdi:battery-lock",
            device_class=None,
            state_class=None,
            unit=None,
            value_fn=lambda c: "on" if c.state_store.plan.prevent_discharge else "off",
            device_type="system",
        ),
        BeemAISensor(
            coordinator, entry,
            ...
            state_class=None,
            unit=None,
            value_fn=lambda c: "advanced" if c.state_store.plan.charge_power_w > 0 else "auto",
            device_type="system",
        ),
        BeemAISensor(
            coordinator, entry,
            ...
            state_class=SensorStateClass.MEASUREMENT,
            unit="%",
            value_fn=lambda c: c.state_store.plan.min_soc,
            device_type="system",
        ),
    ]
